function model_fun(conn,num_frames,parameters)
...
if run_flags > 0
    ...
    % Create progress bar
    fprintf("\n\nIterively training and testing elastic-net model..")

    % Train and test all models
    win_spec_vec = zeros(new_frames,1);
    win_sens_vec = zeros(new_frames,1);
    win_accy_vec = zeros(new_frames,1);
    pat_spec_vec = zeros(new_frames,1);
    pat_sens_vec = zeros(new_frames,1);
    pat_accy_vec = zeros(new_frames,1);
    for iter = 1:new_frames
        ...
        win_spec_vec(iter) = true_pos / num_pos;
        win_sens_vec(iter) = true_neg / num_neg;
        win_accy_vec(iter) = (true_pos + true_neg) / (num_pos + num_neg);

        % Create sensitivity and specificity measurements
        if single_patient_testing
            % Patient-based testing
            pat_spec_vec(iter) = win_spec_vec(iter) >= 0.5;
            pat_sens_vec(iter) = win_sens_vec(iter) >= 0.5;
            pat_accy_vec(iter) = win_accy_vec(iter) >= 0.5;
        else
            ...
            pat_spec_vec(iter) = true_pos / num_pos;
            pat_sens_vec(iter) = true_neg / num_neg;
            pat_accy_vec(iter) = (true_pos + true_neg) / (num_pos + num_neg);
        end
